...
public class OrderRegistCustomerController {
	...
	public String checkOrder(Model model, OrderForm form) {
		...
		while (itr.hasNext()) {
			...
			if (item.getStock() == 0) {// error if 在庫がない場合。
				orderitem.setOrderNum(0);
				String mesage = "※" + orderitem.getName() + "は、在庫切れのため注文できません。";
				error.add(mesage);

			} else if (item.getStock() - basketBean.getOrderNum() < 0) {// (stock - orderNum) < 0, 在庫が足りない場合。
				orderitem.setOrderNum(item.getStock());
